...
from utee.quantize import quantize_kmeans as kmeans
from utee.quantize import quantize_kmeans_nnz as kmeans_nnz
from utee.quantize import quantize_kmeans_fixed_nnz as kmeans_fixed_nnz
from utee.quantize import quantize_kmeans_nnz_fixed_0_center as kmeans_nnz_fixed_0_center


import dataset

from resnet_dense import ResNet34 as CLdense
from resnet_abcv2 import ResNet34 as CLabcv2
from resnet_lr import ResNet34 as CLlr
...
if __name__ == '__main__':
    ...
    parser.add_argument('--defend_iter', type=int, default=7,
                        help="defend iteration for the adversarial sample computation")
    ...
    train_loader, test_loader = dataset.get10(batch_size=args.batch_size, data_root=args.data_root, num_workers=8)
    ...
    try:
        ...
        for epoch in range(args.epochs):
            ...
            message = model_train_proj_prune_admm_quant([model, modelz, modelu], epoch, train_loader, optimizer,
                                                        ...
                                                        nnzfix=args.prune_algo == "baseline",
                                                        param_list=param_list, layernnz=layernnz,
                                                        adv_iter=args.defend_iter, criterion=F.cross_entropy,
                                                        prune_tk=prune_lambda,
                                                        quantize_tk=quantize_lambda, rho=0.1,
                                                        admm_interval=args.quant_interval,
                                                        proj_interval=args.prune_interval, weight_bits=weight_bits)
            ...
            if epoch % args.test_interval == 0:
                ...
                if quantize_lambda is not None:
                    ...
                    print(
                        "\t model dist: {}, layer-wise dist :{}, relative: {}".format(dist, dist_layer, relative_layer))
                    if dist < best_dist:
                        best_dist = dist
